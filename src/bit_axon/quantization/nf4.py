        ...
        for name, child in mod.children().items():
            if isinstance(child, list):
                    ...
                    if isinstance(item, nn.Linear):
                        new_list.append(
                            nn.QuantizedLinear.from_linear(
                                item, group_size=group_size, bits=bits
                            )
                        )
                    else:
                ...
                setattr(
                    ...
                    name,
                    nn.QuantizedLinear.from_linear(
                        child, group_size=group_size, bits=bits
                    ),
                )
